...
This can be done with two components, the Rewriter which is responsible for rewriting the \lsystem to given iteration and the Interpreter which is responsible for interpreting symbols and producing output (Fig. \ref{fig:simpleSystem}).
...
However components in the system \ref{fig:simpleSystem} have too much tasks to do thus they will be complicated to implement and hard to extend and test.
...
The Interpreter will do interpreting of symbols which means keep position of virtual turtle in space, saving and loading of states tsc.
...
The system in Figure \ref{fig:callerComponent} can be enhanced even more.
...
It provides controlled behavior for random number generation described in section \ref{sec:measuring}.
...
\emph{Axiom provider} next extending component and it provides axiom to the Iterator.
Axiom provider is wrapper around single symbol property called axiom.
The Iterator is designed generally to take axiom from any component implementing \texttt{ISymbolProvider} so it is possible to connect for example another Rewriter as axiom provider (Fig. \ref{fig:inputProvider}).
...
Component graph with Inner \lsystem processor component is shown in Figure \ref{fig:innerSystem}.

The Inner \lsystem processor internally works in similar way how the Process manager do (see section \ref{sec:inputProcessing}).
...
The components graph can be specified by special process configuration which needs to be defined in the input\footnote{
	The only implementation of Inner \lsystem processor class \texttt{LsystemInLsystemProcessor} uses process configuration called \emph{InnerLsystemConfig} for creating inner component graph.
	This process configuration must be defined (it is defined in standard library).}.
Interpreter caller in inner component graph is automatically connected to all interpreters in original component graph (see \ref{sec:caller}) therefore inner \lsystem is interpreted by the same interpreter as the main \lsystem.
...
Interpretation of symbol as another \lsystem is demonstrated by Figure \ref{fig:innerLsystem}.
The Pythagoras tree is made of Menger sponges.
Iteration if each Menger sponges depends on its size.
...
Component graph is shown in Figure \ref{fig:finalSystem}.
